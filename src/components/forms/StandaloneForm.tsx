...
export function StandaloneForm({ children, theme, className }: StandaloneFormProps) {
  ...
  return (
    <div 
      ...
      style={{
        ...
        '--shelf-shadow': theme.shadows?.medium || '0 4px 12px rgba(0, 0, 0, 0.1)',
        ...theme.customCSS ? {} : {}
...
function getBackgroundStyle(theme: FormTheme): React.CSSProperties {
  ...
  switch (type) {
    ...
    case 'gradient':
      return { 
        background: gradient || `linear-gradient(135deg, ${theme.primaryColor} 0%, #ffffff 100%)`
      };
