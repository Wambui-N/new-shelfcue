...
export function ThemeEditor() {
  ...
  useEffect(() => {
    if (formData.theme.fontFamily) {
      const font = availableFonts.find(f => f.value === formData.theme.fontFamily)
      if (font?.type === 'google') {
        const link = document.querySelector(`link[data-font="${formData.theme.fontFamily}"]`)
        if (!link) {
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => handleThemeChange('borderRadius', parseInt(e.target.value) || 0)}
              placeholder="8"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              {availableFonts.find(f => f.value === formData.theme.fontFamily)?.type === 'google' 
                ? '✓ Loaded from Google Fonts'
                : availableFonts.find(f => f.value === formData.theme.fontFamily)?.type === 'local'
                ? '✓ Custom font'
                : '✓ System font'}
            </p>
          </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              All theme changes are reflected in real-time on the live preview panel to the left. 
              Try changing colors, fonts, or border radius to see instant updates.
            </p>
          </div>
        </div>
      </Card>
