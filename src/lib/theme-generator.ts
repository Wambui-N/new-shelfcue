 ...
 */
export function createThemeFromBrand(primaryColor: string, fontFamily?: string): FormTheme {
 ...
 */
export function validateTheme(theme: Partial<FormTheme>): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (theme.borderRadius !== undefined && (theme.borderRadius < 0 || theme.borderRadius > 50)) {
